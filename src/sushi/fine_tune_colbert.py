...
def fine_tune_colbert():
    ...
    with Run().context(RunConfig(nranks=1, experiment=experiment_name)):
        config = ColBERTConfig(bsize=1260, lr=1e-05, warmup=20000, doc_maxlen=180, dim=128, attend_to_mask_tokens=False, nway=1260, accumsteps=1, similarity='cosine', use_ib_negatives=True)
